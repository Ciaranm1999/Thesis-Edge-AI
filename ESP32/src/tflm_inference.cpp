...
void setup() {
    ...
    Serial.printf("  Tensor arena     : %6u B  (%4.1f KB)  [heap-allocated]\n",
                  tf.interpreter->arena_used_bytes(),
                  tf.interpreter->arena_used_bytes() / 1024.0f);
    Serial.printf("  Model in flash   : %6u B  (%4.1f KB)  [static array, not heap]\n",
                  g_tflm_model_len, g_tflm_model_len / 1024.0f);
