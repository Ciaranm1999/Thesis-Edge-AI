...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{images/fig_duty_cycle_nodes.png}
    \caption{Gantt-style timing diagram for one 15-minute (900\,s) sensing cycle across all three mesh nodes. The master wakes first, completes a \SI{45}{\second} SGP30 warm-up and its own DHT22/MQ3 readings, then opens an ESP-NOW listen window (maximum \SI{90}{\second}). Both slave nodes run identical firmware: they wake approximately \SI{4}{\second} after the master, complete the same \SI{45}{\second} SGP30 warm-up and sensor reads, then transmit a \SI{3}{\milli\second} ESP-NOW packet (red bars) that is received by the master during its open window. Once all slave packets are received the master closes its listen window early --- typically around \SI{52}{\second} into the cycle --- and sends a timing-correction packet back to each slave to keep the mesh synchronised. All three nodes then return to deep sleep for the remainder of the 900-second cycle. Dashed red arrows indicate the ESP-NOW transmissions from each slave to the master.}
