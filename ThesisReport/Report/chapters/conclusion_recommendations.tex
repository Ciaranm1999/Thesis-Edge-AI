...
A longer-term hardware consideration is whether a dedicated ethanol sensor is necessary at all. The results of this thesis suggest it may not be. The TVOC and ethanol channels follow correlated onset trajectories: both rise monotonically following \textit{Botrytis cinerea} activity, and the Node~2 ethanol signal mirrors the TVOC onset pattern with consistent spatial stratification across batches. This correlation raises the question of whether the ethanol channel adds discriminatory information beyond what TVOC already provides, or whether it is essentially redundant given a sufficiently sensitive broadband VOC sensor. If a high-quality MEMS gas sensor were deployed --- one with good sensitivity to the combined VOC signature of \textit{Botrytis cinerea} metabolites including 1-octen-3-ol and short-chain alcohols --- the MQ3 could be eliminated entirely rather than merely replaced. Removing the ethanol sensor would simplify the node to a single VOC channel plus temperature and humidity, dramatically reduce hardware complexity and cost, eliminate the thermal stabilisation challenge associated with the MQ3 heater, and further reduce the node's physical footprint. An ablation study comparing classifier accuracy with and without the ethanol feature would be a straightforward and informative first step in assessing whether this simplification is viable.
