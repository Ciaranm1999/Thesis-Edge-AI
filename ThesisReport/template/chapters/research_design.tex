...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][7cm][c]{0.72\textwidth}{\centering\small\textit{[Insert photo of assembled sensor node PCB here]}}}
...
To reduce idle power consumption, all nodes disabled Bluetooth at startup and reduced the CPU clock from 240\,MHz to 160\,MHz, as the lower frequency is sufficient for all sensing and ML operations while reducing dynamic power draw.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][8cm][c]{0.72\textwidth}{\centering\small\textit{[Insert photo of experimental setup here: chamber, sensor nodes at their three positions, Raspberry Pi 5 camera mounted above]}\vspace{0.5em}}}
...
A Raspberry Pi 5 with a camera module was mounted above the chamber and configured to capture an image of the strawberry batch at one-hour intervals. This camera served exclusively as a ground-truth labelling instrument. A reading was labelled as high risk (class 1) if any mould was visible in the corresponding image, including early microscopic growth at the surface of the fruit; readings captured before any mould appeared were labelled low risk (class 0). The label transition was applied at the earliest image in which any mould was visible, so that the positive-class sensor readings corresponded to the environmental signature of mould onset rather than established spoilage.
...
\begin{figure}[H]
    \centering
    \fbox{\parbox[c][7cm][c]{0.72\textwidth}{\centering\small\textit{[Insert photo of PPK2 measurement setup here: PPK2 connected to sensor node, showing supply and measurement configuration]}}}
...
Data was collected across five separate experimental batches. Batches 1 through 4 (1278 samples: 607 mould, 671 no-mould) formed the combined training pool. Batch 5 (332 samples) was withheld as the held-out test set and was never used during any stage of training or validation for any framework, ensuring an uncontaminated evaluation.

\subsection{Feature Engineering}

Raw sensor readings were not passed directly to the network. Each input feature was normalised to the range $[0, 1]$ using min-max scaling fitted on the training set (Batches 1--4):
...
The network was trained offline on a desktop computer using TensorFlow on the full combined training dataset (Batches 1--4). The trained model was post-training quantised to 8-bit integer weights and converted to the \texttt{.tflite} format. The quantised model was embedded in the ESP32 firmware as a constant byte array and loaded into SRAM at runtime. TF Lite Micro performed one forward pass per sensing cycle; no weights were modified after deployment.
...
For AIfES, no pre-trained weights were used. All 193 network parameters were initialised on the ESP32 using Glorot uniform initialisation and trained entirely on-device. The optimiser was Adam with a learning rate of 0.001 and binary cross-entropy loss. Training ran for 20 epochs over the full combined training dataset (1278 samples) using a mini-batch size of 4. A per-epoch Fisher-Yates shuffle was applied to the training data using the ESP32 hardware random number generator.
